...
class Separator(nn.Module):
    def __init__(self, imsize, converts, ch=512, down_scale=2):
...
class Generator(nn.Module):
    def __init__(self, input_channels=512):  # Match your encoder output channels
        super().__init__()
        self.model = nn.Sequential(
    ...
    def forward(self, content, style):
        x = content + style
        out = self.model(x)
        out = F.interpolate(out, size=(48, 48), mode='bilinear', align_corners=False)
        return out
